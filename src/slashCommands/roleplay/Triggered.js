...
module.exports = {
	name: 'triggered',
	description: 'Sends a gif of someone who is triggered about something',
	category: 'roleplay',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/triggered/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} is triggered`);
