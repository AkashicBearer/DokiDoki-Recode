...
module.exports = {
	name: 'peek',
	alias: 'lurk',
	description: 'Sends a gif of something peeking at something ',
	category: 'roleplay',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/peek/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} is lurking around`);
