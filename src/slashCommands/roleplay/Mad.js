...
module.exports = {
	name: 'mad',
	description: 'Show someone you are mad at them',
	category: 'roleplay',
	options: [
		{
			name: 'user',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/mad/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} is mad at ${user.username}`);
