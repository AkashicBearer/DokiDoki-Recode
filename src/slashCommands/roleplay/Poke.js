...
module.exports = {
	name: 'poke',
	description: 'Poke a hole in someone or just wake them up',
	category: 'roleplay',
	options: [
		{
			name: 'user',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/poke/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} pokes ${user.username}`);
