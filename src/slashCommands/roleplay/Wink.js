...
module.exports = {
	name: 'wink',
	description: 'Wink at someone, you know?',
	category: 'roleplay',
	options: [
		{
			name: 'user',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/wink/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} winks at ${user.username}`);
