...
module.exports = {
	name: 'happy',
	description: 'Sends a gif of someone who is happy/excited',
	category: 'roleplay',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/happy/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} is excited about something`);
