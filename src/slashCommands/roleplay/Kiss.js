...
module.exports = {
	name: 'kiss',
	description: 'Kiss someone you simp',
	category: 'roleplay',
	options: [
		{
			name: 'user',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/kiss/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} gives a kiss to ${user.username}`);
