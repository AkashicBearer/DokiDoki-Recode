...
module.exports = {
	name: 'wakeup',
	description: 'Sends a gif of someone waking up',
	category: 'roleplay',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch('https://kawaii.red/api/gif/alarm/token=193021560792154112.xEZWOoQDrC5wIUliv2UG&type=json/')
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} wakesup`);
