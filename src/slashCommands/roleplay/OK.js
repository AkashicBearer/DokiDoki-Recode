...
module.exports = {
	name: 'ok',
	description: 'Sends a gif of someone doing the ok handsign',
	category: 'roleplay',
	...
	run: async (client, interaction) => {
		try {
			...
			const img = await fetch(`https://kawaii.red/api/gif/ok/token=${Token}&type=json/`)
				.then(res => res.json()).catch(err => {
					console.log(err);
				});

			ImageEmbed.setTitle(`❯ ${interaction.user.username} send an Ok`);
